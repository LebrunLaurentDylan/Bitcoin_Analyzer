...
def compute_moving_average_for_rates_data(rates, nb_days_interval):
    ...
    inferieur à nb_days_interval sauf quand len(index) inférieur à 2 ; ou on prend juste la valeur et c'est tout.
    Ensuite sauvegarder une somme "s" des (valeurs) à chaque index rajouter la somme et quand
    index est >= nb_days_interval rajouter la somme suivante puis retirer la première valeur avant de diviser
    par nb_days_interval chaque somme S ainsi produite sera rajoutée à une variable
    'averages' = [{'date':..date_str, 'value':..somme}] qui sera la variable retournée par la fonction
    "compute_moving_average_for_rates_data" """
